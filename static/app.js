...
document.querySelectorAll('input[name="tipo"]').forEach(r => {
  r.addEventListener('change', () => {
    document.getElementById('outro-wrap').style.display = r.value === 'Outro' ? 'block' : 'none';
  });
});
...
async function submitContato(e) {
  ...
  const body = {
    ...
    cidade_estado:     document.getElementById('c-cidade').value.trim(),
    tipo_contato:      document.querySelector('input[name="tipo"]:checked')?.value || '',
    outro_contato:     document.getElementById('c-outro').value.trim(),
    assunto:           document.getElementById('c-assunto').value.trim(),
    descricao:         document.getElementById('c-descricao').value.trim(),
    canal_resposta:    document.querySelector('input[name="canal"]:checked')?.value || 'E-mail',
    lgpd_consentimento: document.getElementById('c-lgpd').checked,
    receber_informativos: document.getElementById('c-news').checked,
  };
  ...
  try {
    ...
    if (res.ok && data.success) {
      ...
      msgEl.textContent = data.detail || data.message || 'Erro ao enviar.';
    }
  } catch {
    msgEl.className = 'form-msg error';
